...
const EditeClient = ({ client }: { client: Client }) => {
  ...
  const schema = z.object({
    ...
    activite: z.string(),
    numero: z.string(),
    dateInscription: z.string().refine((val) => !isNaN(Date.parse(val)), {
  ...
  const form = useForm<z.infer<typeof schema>>({
    ...
    defaultValues: {
      ...
      dateInscription: client.dateInscription
        ? format(new Date(client.dateInscription), "yyyy-MM-dd")
        : ""
    }
  });
  ...
  function onSubmit(values: z.infer<typeof schema>) {
    transition(() => {
      updateClient(client.id, values)
        .then(() => {
          toast.success("Client edité avec succès");
